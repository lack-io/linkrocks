...
impl Context {
    ...
    pub fn with_parent(ctx: &RefContext, timeout: Option<Duration>) -> (Context, Handle) {
    ...
    /// calling cancel() on the Handle that was returned during inital construction.
...
impl RefContext {
    ...
    pub fn new() -> (RefContext, Handle) {
        ...
        (RefContext::from(ctx), handle)
    }

    /// Builds a new Context. The `done` method returns a future that will complete when
    /// either the handle is cancelled, or when the supplied timeout has elapsed.
    pub fn with_timeout(timeout: Duration) -> (RefContext, Handle) {
        let (ctx, handle) = Context::with_timeout(timeout);
        (RefContext::from(ctx), handle)
    }
    ...
    /// optional timeout has elapsed, the parent context is cancelled, or the optional parent timeout
    /// has elapsed.
    pub fn with_parent(ctx: &RefContext, timeout: Option<Duration>) -> (RefContext, Handle) {
        let (context, handle) = Context::with_parent(ctx, timeout);
        (RefContext::from(context), handle)
    }

    /// Blocks until either the provided timeout elapsed, or a cancel signal is received from
    /// calling `cancel` on the Handle that was returned during inital construction.
    pub fn done(&mut self) -> Pin<Box<dyn Future<Output = ()> + '_ + Send>> {
...
impl From<Context> for RefContext {
    fn from(cx: Context) -> Self {
        RefContext(Arc::new(Mutex::new(cx)))
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn cancelling_mul_level_ctx_cancels_child() {
        // Note that we can't simply drop the handle here or the context will be cancelled.
        let (mut parent_ctx, parent_handle) = RefContext::new();
        let (mut ctx, _) = RefContext::with_parent(&parent_ctx, None);
        let (mut sub_ctx, _) = Context::with_parent(&ctx, None);

        parent_handle.cancel();

        tokio::select! {
            _ = parent_ctx.done() => assert!(true),
            _ = tokio::time::sleep(Duration::from_millis(15)) => assert!(false),
        }
