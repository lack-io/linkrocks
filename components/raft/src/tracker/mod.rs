...
use raftpb::prelude::ConfState;
pub use state::ProgressState;

use crate::{
    confchange::{MapChange, MapChangeType},
    quorum::{AckedIndexer, Index, JointConfig, VoteResult},
    DefaultHashBuilder, HashMap, HashSet,
};

use self::progres::Progress;
...
/// Display and crate::intertools used only of test
#[cfg(test)]
...
impl Configuration {
    ...
    pub fn to_conf_state(&self) -> ConfState {
        ...
        state.voters = self.voters.incoming.raw_slice();
        state.voters_outgoing = self.voters.outgoing.raw_slice();
        state.learners = self.learners.iter().cloned().collect();
        state.learners_next = self.learners_next.iter().cloned().collect();
        state
    }
...
/// `ProgressTracker` contains serveral `Progress`es,
...
impl ProgressTracker {
    ...
    /// Returns an iterator acroess all the node and their progress.
    ///
    ...
    /// Returns a mutable interator across all the nodes and their progress.
    ///
    /// **Note:** Do not use this for majority/quorum calcuation. The Raft node may be
    ...
    /// Returns the maximal commited index for the cluster. The bool flag indicates whether
    ...
    /// Returns the Condidate's eligibility in the current election.
    ///
    ...
    ///
    /// This is the only correct way to verify you have reached a quorum the whole group.
    #[inline]
        ...
        for (id, change_type) in changes {
            match change_type {
                MapChangeType::Add => {
                    ...
                    // Otherwise, CheckQuorum may cause use to step down if it is invoked
