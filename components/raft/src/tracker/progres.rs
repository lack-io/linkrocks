...
use super::{Inflights, ProgressState};
...
pub struct Progress {
    ...
    /// per heartbeat internal. It also probes actual progress of the follower.
    ///
    /// When in ProgressStateReplicate, leader optimistically increases next
    /// to the latest entry send after sending replication message. This is
    ...
    /// RecentActive can be reset to false after an election timout.
    pub recent_active: bool,

    /// Inflight is a sliding window for the inflight messages.
    /// When inflights is full, no more message should be sent.
    /// When a leader sends out a message, the index of the last
    /// entry should be added to inflights. The index MUST be added
    /// into inflights in order.
    /// When a leader receives a reply, the previous inflight should
    /// be freed by calling inflights.free_to.
    pub ins: Inflights,
...
impl Progress {
    ...
    /// Optimistcally advance the index
    #[inline]
    ...
    pub fn maybe_decr_to(&mut self, rejected: u64, match_hint: u64, request_snapshot: u64) -> bool {
        ...
        // The rejection must be state if "rejected" does not match next - 1.
        ...
        if request_snapshot == INVALID_INDEX {
            ...
            // Allow requesting snapshot even if it's not Replication
            self.pending_request_snapshot = request_snapshot;
        }
        self.resume();
        true
    ...
    pub fn update_state(&mut self, last: u64) {
        match self.state {
            ...
            ProgressState::Replicate => self.pause(),
            ProgressState::Snapshot => {
                panic!("updating progress state n unhandled state {:?}", self.state)
            }
        }
    }
}
