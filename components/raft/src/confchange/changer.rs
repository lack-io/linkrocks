use raftpb::prelude::{ConfChangeSingle, ConfChangeType};
...
impl Changer<'_> {
    ...
    pub fn leave_joint(&self) -> Result<(Configuration, MapChange)> {
        if !super::joint(self.tracker.conf()) {
            return Err(Error::ConfChangeError(
                "can't leave a no-joint config".to_owned(),
            ));
        }
        let (mut cfg, mut prs) = self.check_and_copy()?;
    ...
    /// Carries iyt a serial of configuration changes that (in aggregate) mutates the
    /// incoming majority config `Voters[0]` by at most one. This method will return an
    /// error if that is not the case, if the resulting quorum is zoro, or if the
    ...
    /// made to the incoming majority config. Outgoing is eigher empty or preserves the
    ...
    /// beacuse then we'd have to track a peer as a voter and learner simultaneously.
    ...
    /// flags. Addition and Removals should be done after everything is checked OK.
...
fn check_invariants(cfg: &Configuration, prs: &IncrChangeMap) -> Result<()> {
    ...
    // during tests). Instead of having to hand-core this, we allow
    // transitioning from an empty config into any legal and non-empty
    // config.
    ...
    for id in &cfg.learners {
        ...
        // Conversely Learners and Votes doesn't intersect at all.
        if cfg.voters().outgoing.contains(id) {
            return Err(Error::ConfChangeError(format!(
                "{} is in learners and outgoing votes",
                id
            )));
        }
    ...
    for id in &cfg.learners_next {
        ...
        // to a confliciting voter in the outgoing config.
        if !cfg.voters().outgoing.contains(id) {
            return Err(Error::ConfChangeError(format!(
    ...
    if !super::joint(cfg) {
        ...
        if !cfg.learners_next.is_empty() {
            return Err(Error::ConfChangeError(
                "learners_next must be empty when no joint".to_owned(),
            ));
        }
        if cfg.auto_leave {
            return Err(Error::ConfChangeError(
