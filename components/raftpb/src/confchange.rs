...
impl ConfChangeV2 {
    ...
    ///
    /// It will return Some if any only if this config change will use Joint Consensus.
    /// which is the case if contains more than one change or if the use of Joint
    ...
    pub fn enter_joint(&self) -> Option<bool> {
        ...
        if self.transition() != ConfChangeTransition::Auto || self.changes.len() > 1 {
            match self.transition() {
                ...
                ConfChangeTransition::JointExplicit => Some(false),
            }
        } else {
            None
        }
